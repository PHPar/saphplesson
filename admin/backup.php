...
$zipfile = new zipfile();
    ...
    if($_POST["type"]=="sql"){
      ...
      $dbname = $SQL_DB;
      $filesno = intval($_POST["filesno"]);

      $TablesArray = array();
         if($filesno=="" or filesno < 0){
             $filesno = "1";
         }
                $dump_buffer="";
                ...
                if($num_tables == 0)
                {
                        echo "# áã íÊã ÇáÚËæÑ Úáì ÌÏÇæá";
                        exit;
                }

                $dump_buffer.= "# SaphpLesson 4.0 $crlf";
                $dump_buffer.= "# Êã ÇÎÐ åÐå ÇáäÓÎå ÈÊÇÑíÎ :$crlf";
                $dump_buffer.= "# ".date("F j, Y, g:i a")."$crlf";
                $dump_buffer.= "# ÞÇÚÏÉ ÇáÈíÇäÇÊ : $dbname$crlf";
                $dump_buffer.= "# WebSite : http://www.saphplesson.org $crlf";
                $dump_buffer.= "# WebSite : http://www.dt-live.com $crlf";
                ...
                while($i < $num_tables)
                {
                        ...
                        if ($_POST[$table."2"]){
                            if($_POST["multi"]=="3"){
                                ...
                                $dump_buffer.= "# ÇáäÙã ÇáåíßáíÉ ááÌÏæá '$table'$crlf";
                                $dump_buffer.= "#$crlf$crlf";
                                $db = $table;
                                $dump_buffer.= get_table_def($table, $crlf,$dbname).";$crlf";
                                $dump_buffer.= "$crlf#$crlf";
                                $dump_buffer.= "# ÇÓÊÑÌÇÚ ÈíÇäÇÊ ÇáÌÏæá '$table'$crlf";
                                $dump_buffer.= "#$crlf$crlf";
                                $tmp_buffer="";
                                get_table_content($dbname, $table, 0, 0, 'my_handler', $dbname);
                                $dump_buffer.=$tmp_buffer;
                                $TablesArray[$i]["dump"] = $dump_buffer;
                                $TablesArray[$i]["name"] = $table;
                         }
                        $i++;
                        $dump_buffer.= "$crlf";
                }
                if($_POST["multi"]=="3"){
                    ...
                    for($i=0; $i <= $count; $i++){
                        if($TablesArray[$i]["name"]){
                            $zipfile -> addFile($TablesArray[$i]["dump"],$TablesArray[$i]["name"].".sql");
                        }
                    }
                 echo $zipfile -> file();
                }elseif($filesno=="1" and $_POST["multi"]!="3"){
                if($_POST["type"]=="zip"){
                    $zipfile -> addFile($dump_buffer, "SaphpLesson.sql");
                    echo $zipfile -> file();
                }
                ...
                echo $dump_buffer;
                    ...
                    $new_buffer = explode("$crlf" , $dump_buffer);
                    $count = count($new_buffer);
                    $lineno = ceil($count / $filesno);
                    $x = 0;
                    $s = 1;
                    $buffer = "";
                    for($i=0; $i <= $count; $i++){
                    ...
                    $buffer .= $crlf;
                    ++$x;
                        if($x==$lineno or $i>=$count){
                           $zipfile -> addFile($buffer, "SaphpLesson-".$s.".sql");
                           $buffer = "";
                           $x = 0;
                           ++$s;
                        }
...
else
{
      ...
      $dbname = $SQL_DB;
         ...
         while ($Data = mysql_fetch_array($Result))
         {
              ...
              $Data["SizeKB"]=round($Size/1024,2);
              $Siize = $Siize + $Data["SizeKB"];
              $table_row[] = $Data;

              $DBSize=$DBSize+$Size;
              $DBRows=$DBRows+$Data[Rows];
         }
            echo $tpl->display("backup.tpl");
}
        ...
        {
                $result = mysql_query('SELECT * FROM ' . $db . '.' . $table . $add_query)
or die();
                if ($result != false) {
