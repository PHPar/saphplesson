...
if(!class_exists('TCPDF')) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.1.001_PHP4 (http://www.tcpdf.org)');
	...
	*/
    ...
    /**
		...
		function TCPDF($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding="UTF-8") {
			...
			$this->u = 0;
			$this->HREF = '';
			$this->fontlist = array("almohanad", "courier");
		...
		*/
		...
		function Header() {
			if ($this->print_header) {
				...
				$this->SetX($header_x);
				$this->Cell($this->header_width, $cell_height, $this->header_title, 0, 1, 'C');
				// header string
		...
		function Footer() {
			if ($this->print_footer) {
				...
				if ($this->rtl) {
                    ...
                    $this->ln();
                    $this->Cell(0, $footer_height, 'Powered by SaphpLesson 4.0', 0, 0, 'C',0,'http://www.saphplesson.org');
				} else {
					$this->SetX($this->original_lMargin);
		...
		* @see SetFillColor()
		*/
		function SetFillColorArray($color, $storeprev=false) {
			if (isset($color)) {
				$color = array_values($color);
				$r = isset($color[0]) ? $color[0] : -1;
				$g = isset($color[1]) ? $color[1] : -1;
		...
		* @see SetFillColor()
		*/
		function SetTextColorArray($color, $storeprev=false) {
			if (isset($color)) {
				$color = array_values($color);
				$r = isset($color[0]) ? $color[0] : -1;
				$g = isset($color[1]) ? $color[1] : -1;
	 ...
	 /**
		...
		*/
		function _parsepng($file) {
			$f = fopen($file,'rb');
			if(empty($f)) {
				$this->Error('Can\'t open image png file: '.$file);
			}
			//Check signature
